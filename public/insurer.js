...
el("loadSessionBtn").addEventListener("click", async () => {
  ...
  try {
    ...
    box.innerHTML = `
      <div class="terms-box__row"><span>Patient Alias</span><strong>${terms.patient}</strong></div>
      <div class="terms-box__row"><span>Therapist Alias</span><strong>${terms.therapist}</strong></div>
    ...
    // Also refresh confirmation status in case terms were already published earlier.
    await refreshStatus();
  } catch (err) {
    box.className = "terms-box terms-box--empty";
    box.textContent = "Could not load this session — check the Session ID with the therapist.";
  ...
  try {
    ...
    box.innerHTML = `
      <table class="invoice-table">
        <tr class="highlight"><td>Patient would pay</td><td>CHF ${rappenToChf(preview.patientAmount)}</td></tr>
...
el("confirmBtn").addEventListener("click", async () => {
  ...
  btn.disabled = true;
  const pendingEntry = addLedgerEntry({ title: "Submitting insurer confirmation…", status: "pending" });
  try {
    ...
    pendingEntry.remove();
    addLedgerEntry({ title: "SESSION_CONFIRMED · insurer", meta: `hash ${result.termsHash}` });
    el("statusBox").className = "terms-box";
    el("statusBox").innerHTML = `<div class="terms-box__row"><span>Status</span><strong>Terms published — waiting on patient &amp; therapist</strong></div>`;
  } catch (err) {
    pendingEntry.remove();
...
async function refreshStatus() {
  ...
  try {
    ...
    if (status.insurerConfirmed) {
      el("statusBox").className = "terms-box";
      const waitingOn = [];
      if (!status.patientConfirmed) waitingOn.push("patient");
      if (!status.therapistConfirmed) waitingOn.push("therapist");
      el("statusBox").innerHTML = `<div class="terms-box__row"><span>Status</span><strong>${
        waitingOn.length === 0 ? "All parties confirmed" : `Terms published — waiting on ${waitingOn.join(" & ")}`
      }</strong></div>`;
    }
  } catch {
